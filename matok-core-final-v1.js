(() => {
  'use strict';
  const VERSION='20260902-final-core-2';
  ...
  function renderAdminScheduleFinal(){
    ...
    box.innerHTML=`<div class="mfScheduleGrid">${DAYS.map(([day,slots])=>`<article class="mfDay"><header><b>${day}</b><span>${slots.length===2?'בוקר · ערב':'שישי'}</span></header>${slots.map(slot=>{
      const c=cfg(slot),rows=assFor(slot),missing=Math.max(0,c.required_count-rows.length);
      const assignedIds=new Set(rows.map(a=>String(a.staff_id)));
      const available=adminState.availability.filter(a=>a.slot_key===slot&&a.status!=='unavailable'&&!assignedIds.has(String(a.staff_id)));
      return `<section class="mfShift"><div class="mfShiftHead"><div><b>${slotName(slot)}</b><small>${esc(c.start_time)}–${esc(c.end_time)}</small></div><span class="badge ${missing?'bad':'ok'}">${rows.length}/${c.required_count}</span></div><div class="mfAvailability">${available.length?available.slice(0,8).map(a=>`<span class="mfAvailChip ${a.status}">${esc(a.full_name)}${a.status==='preferred'?' ★':''}</span>`).join(''):'<span class="mfAvailEmpty">אין עובדים זמינים נוספים</span>'}</div><div class="mfAssigned">${rows.length?rows.map(a=>{const s=staffById(a.staff_id);return `<div class="mfPerson"><div><b>${esc(s?.full_name||'עובד')}</b><small>${esc(a.role_name||s?.role_name||'מכירה')}</small></div><button class="btn danger" data-mf-remove="${esc(a.staff_id)}" data-mf-slot="${esc(slot)}">−</button></div>`}).join(''):'<div class="mfEmpty">אין עובדים במשמרת</div>'}</div><button class="btn primary" style="width:100%" data-mf-add="${esc(slot)}">+ הוספת עובד</button></section>`}).join('')}</article>`).join('')}</div>`;
  ...
  function renderCandidateFinal(){
    const list=document.getElementById('mfCandidateList');if(!list)return;
    const on=new Set(assFor(activeCandidateSlot).map(a=>String(a.staff_id)));
    const candidates=adminState.staff.filter(s=>!on.has(String(s.id)));
    list.innerHTML=candidates.length?candidates.map(s=>{const av=avFor(activeCandidateSlot,s.id),configured=configuredFor(s,activeCandidateSlot);let text=av==='unavailable'?'סימנה שלא יכולה — אפשר לשבץ בכל זאת':av==='preferred'?'מעדיפה משמרת זו':av==='available'?'זמינה':!configured?'לא ביום/סוג המשמרת הקבוע — אפשר לשבץ בכל זאת':'לא הוגשה זמינות — אפשר לשבץ';const warn=av==='unavailable'||!configured||!av;return `<div class="mfCandidate ${warn?'warning':''}"><div><b>${esc(s.full_name)}</b><small>${esc(s.role_name||'מכירה')} · ${esc(text)}</small>${warn?'<span class="mfOverride">הרשאת מנהל עוקפת את ההגבלה</span>':''}</div><button class="btn primary" data-mf-cand="${esc(s.id)}" data-mf-act="add">שיבוץ</button></div>`}).join(''):'<div class="mfEmpty">כל העובדים כבר שובצו במשמרת זו.</div>';
    list.querySelectorAll('[data-mf-cand]').forEach(b=>b.onclick=()=>setAssignmentFinal(activeCandidateSlot,b.dataset.mfCand,true,b,true));
  }
  ...
  async function renderAdminAvailabilityFinal(){
    const box=document.getElementById('mfAvailabilityAdmin');if(!box)return;
    if(adminState.week?.status==='published'){
      box.innerHTML='<div class="mfPublishedAvailabilityHidden"><b>השבוע כבר פורסם.</b><br>הגשות הזמינות של השבוע נשמרו להיסטוריה ולסיכום, אבל אינן מוצגות יותר כתור עבודה פעיל. להגשות חדשות עוברים לשבוע הבא.<div style="margin-top:9px"><button type="button" class="btn primary" id="mfGoNextAvailabilityWeek">מעבר לשבוע הבא</button></div></div>';
      document.getElementById('mfGoNextAvailabilityWeek').onclick=()=>document.getElementById('mfNextWeek')?.click();
      return;
    }
    const rows=adminState.availability||[];const by=new Map();
    rows.forEach(r=>{if(!by.has(r.staff_id))by.set(r.staff_id,{name:r.full_name,note:r.general_note,submitted:r.submitted_at,items:[]});by.get(r.staff_id).items.push(r)});
    box.innerHTML=by.size?[...by.values()].map(x=>`<div class="mfMessage"><div class="mfMessageHead"><b>${esc(x.name)}</b><span class="mfStatus ok">הוגש</span></div><div class="mfAvailability">${x.items.map(a=>`<span class="mfAvailChip ${a.status}">${esc(labelSlot(a.slot_key))} · ${a.status==='preferred'?'מעדיפה':a.status==='available'?'זמינה':'לא יכולה'}</span>`).join('')}</div>${x.note?`<p>${esc(x.note)}</p>`:''}</div>`).join(''):'<div class="mfEmpty">אין הגשות זמינות לשבוע הזה.</div>';
  }
