(() => {
  ...
  async function loadProfiles(){
    ...
    if(error){console.error(error);box.textContent='טעינת הנתונים נכשלה';return}
    profiles=data||[];
    box.innerHTML='<p>עובדים לא פעילים נשמרים בארכיון, וניתן לשייך להם תלושים ודוחות.</p><table><thead><tr><th>עובד</th><th>מצב</th><th>₪ לשעה</th><th>תחילת עבודה</th><th>הערה</th><th></th></tr></thead><tbody>'
      +profiles.map(r=>'<tr><td><b>'+esc(r.full_name)+'</b></td><td><span class="badge '+(r.is_active?'ok':'wait')+'">'+(r.is_active?'פעיל':'לא פעיל')+'</span></td><td><input type="number" step="0.01" value="'+Number(r.hourly_rate||0)+'" data-rate="'+r.staff_id+'"></td><td><input type="date" value="'+(r.employment_start||'')+'" data-start="'+r.staff_id+'"></td><td><input value="'+esc(r.notes||'')+'" data-note="'+r.staff_id+'"></td><td><button class="btn secondary" data-save-profile="'+r.staff_id+'">שמירה</button></td></tr>').join('')+'</tbody></table>';
    box.querySelectorAll('[data-save-profile]').forEach(b=>b.onclick=async()=>{
      const id=b.dataset.saveProfile,rate=Number(box.querySelector('[data-rate="'+id+'"]').value||0),start=box.querySelector('[data-start="'+id+'"]').value||null,note=box.querySelector('[data-note="'+id+'"]').value||'';
      const {error}=await supabaseClient.rpc('admin_save_payroll_profile',{p_staff_id:id,p_hourly_rate:rate,p_employment_start:start,p_notes:note});
      toast?.(error?'השמירה נכשלה':'נתוני השכר נשמרו');
    });
  }
  async function fillStaffSelect(){
    await getStaff(true);const s=document.getElementById('mpHoursStaff');
    if(!s)return;
    const before=s.value;
    s.innerHTML=['פעילים','לא פעילים'].map((label,i)=>'<optgroup label="'+label+'">'+staff.filter(x=>Boolean(x.is_active)===(i===0)).map(x=>'<option value="'+x.id+'">'+esc(x.full_name)+(x.is_active?'':' · לא פעיל')+'</option>').join('')+'</optgroup>').join('');
    if([...s.options].some(x=>x.value===before))s.value=before;
  }
  ...
  async function importHours(ev){
    ...
    try{
      await ensureXlsx();await getStaff(true);
  ...
  async function loadDelivery(){
    await getStaff(true);
    const box=document.getElementById('mpDelivery');if(!box)return;
    const period=document.getElementById('mpDeliveryPeriod')?.value||periodNow();
    const {data,error}=await supabaseClient.rpc('admin_list_employee_documents_v2',{p_period:period});
    if(error){box.textContent='טעינת המסמכים נכשלה';console.error(error);return}
    docRows=data||[];
    box.innerHTML='<p style="font-size:12px;margin-bottom:10px">WhatsApp פותח הודעה מוכנה בלבד. אחרי ששלחת את ההודעה בפועל, לחץ „סימון כנמסר” לצורך רישום מדויק.</p>'+
    '<table><thead><tr><th>עובד</th><th>מסמך</th><th>חודש</th><th>סטטוס</th><th>פעולות</th></tr></thead><tbody>'+
    docRows.map(r=>'<tr><td><b>'+esc(r.full_name)+'</b>'+(staff.find(x=>x.id===r.staff_id)?.is_active===false?'<small> · לא פעיל (ארכיון)</small>':'')+'</td><td>'+(r.doc_type==='payslip'?'תלוש שכר':'דוח שעות')+'</td><td>'+esc(r.period_label)+'</td><td>'+(r.delivered_at?'נמסר':r.reviewed_at?'נבדק':'חדש')+'</td><td><div class="actions"><button class="btn secondary" data-view-doc="'+r.id+'">פתיחה</button><button class="btn secondary" data-review-doc="'+r.id+'">נבדק</button><button class="btn secondary" data-wa-doc="'+r.id+'">WhatsApp</button><button class="btn primary" data-delivered-doc="'+r.id+'" '+(r.delivered_at?'disabled':'')+'>סימון כנמסר</button></div></td></tr>').join('')+
    '</tbody></table>';
    box.querySelectorAll('[data-view-doc]').forEach(b=>b.onclick=()=>openAdminDoc(b.dataset.viewDoc));
    ...
    box.querySelectorAll('[data-delivered-doc]').forEach(b=>b.onclick=async()=>{
      if(!confirm('האם המסמך נשלח או נמסר לעובד בפועל?'))return;
      b.disabled=true;const r=await supabaseClient.rpc('admin_mark_employee_document',{p_document_id:b.dataset.deliveredDoc,p_action:'delivered'});
      if(r.error){toast?.('רישום המסירה נכשל');b.disabled=false;return}
      loadDelivery();
    });
  }
  async function exportDelivery(){
    ...
    await loadDelivery();
    await ensureXlsx();
    const rows=docRows.map(r=>({'עובד':r.full_name,'סוג':r.doc_type==='payslip'?'תלוש שכר':'דוח שעות','חודש':r.period_label,'סטטוס':r.delivered_at?'נמסר':r.reviewed_at?'נבדק':'חדש'}));
    const ws=XLSX.utils.json_to_sheet(rows.length?rows:[{'עובד':'','סוג':'','חודש':'','סטטוס':''}]);
    const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'מסירת מסמכים');
    XLSX.writeFile(wb,'דוח_מסירת_תלושים_ודוחות_'+(document.getElementById('mpDeliveryPeriod')?.value||periodNow())+'.xlsx');
  }
  ...
  function printPayrollPolicy(){
    ...
    w.document.open();w.document.write('<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><title>MATOK BASIC - נוהל שכר</title><style>body{font-family:Arial,sans-serif;margin:35px;line-height:1.9;color:#1a1a2e}h2,h3{text-align:center}</style></head><body>'+body+'</body></html>');w.document.close();w.focus();w.print();
  }
