(() => {
  'use strict';
  const VERSION='20260819-final-manager-home-1';
  ...
  function ensure(){
    ...
    overview.prepend(root);document.getElementById('mfManagerWeeksRefresh').onclick=load;return root;
  }
  ...
  let timer=null;new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(()=>{if(ensure())load()},80)}).observe(document.documentElement,{childList:true,subtree:true});
  window.addEventListener('focus',()=>{if(isAdmin())load()});
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(load,180),{once:true});else setTimeout(load,100);
})();